...
const emptyBodyDraft = (): BodyDraft => ({ recorded_at: localToday(), weight: 0, body_fat: 0, muscle: 0, fat_mass: null, lean_mass: null, body_water: null, bmi: null, basal_metabolic_rate: null });
...
const detailCardOrder = ["sugars", "saturated_fat", "trans_fat", "cholesterol", "sodium", "fiber", "potassium", "vitamin_a", "vitamin_c", "calcium", "iron"] as const;
const bodyMetricMeta: Record<BodyMetric,{label:string;unit:string}> = { weight:{label:"Peso",unit:"kg"}, body_fat:{label:"Grasa corporal",unit:"%"}, muscle:{label:"Masa muscular",unit:"kg"}, lean_mass:{label:"Masa libre de grasa",unit:"kg"} };
...
function dailyNutritionTips(total: Macros, details: NutritionDetails, goals: Macros | null, hasMeals: boolean) {
  if (!hasMeals) return [{ tone:"neutral", title:"Empieza el registro", detail:"Añade una comida para ver consejos personalizados.", fact:"Dato: registrar también cantidades pequeñas mejora la comparación diaria." }];
  if (!goals) return [{ tone:"neutral", title:"Configura tus objetivos", detail:"Así podremos comparar tu día y darte referencias útiles.", fact:"Dato: los objetivos se ajustan al contexto personal, no solo al peso." }];
  const tips: { tone:"positive" | "attention" | "neutral"; title:string; detail:string; fact:string }[] = [];
  const saturatedLimit = goals.calories * .1 / 9;
  if (total.protein < goals.protein * .65) tips.push({ tone:"attention", title:"Proteína por reforzar", detail:`Llevas ${fmt(total.protein)} de ${fmt(goals.protein)} g.`, fact:"Dato: la proteína contribuye al mantenimiento de la masa muscular." });
  if (details.fiber < 25) tips.push({ tone:"attention", title:"Fibra por debajo", detail:`Llevas ${fmt(details.fiber)} de 25 g.`, fact:"Dato: legumbres, fruta, verdura y cereales integrales son fuentes habituales de fibra." });
  if (details.salt > 5) tips.push({ tone:"attention", title:"Sal elevada", detail:`${fmt(details.salt)} g frente a 5 g de referencia.`, fact:"Dato: el sodio es un mineral, no una proteína; 1 g de sodio equivale aproximadamente a 2,5 g de sal." });
  if (details.saturated_fat > saturatedLimit) tips.push({ tone:"attention", title:"Saturadas elevadas", detail:`${fmt(details.saturated_fat)} g frente a ${fmt(saturatedLimit)} g.`, fact:"Dato: las grasas saturadas y trans pertenecen a la familia de las grasas, por eso usan el mismo color." });
  if (total.calories > goals.calories * 1.1) tips.push({ tone:"attention", title:"Calorías por encima", detail:`${fmt(total.calories)} de ${fmt(goals.calories)} kcal.`, fact:"Dato: el balance se interpreta mejor mirando varios días, no una sola comida." });
  if (!tips.length) tips.push({ tone:"positive", title:"Vas bien encaminado", detail:"Los indicadores registrados están dentro de tus referencias.", fact:"Dato: la regularidad del registro hace más útil la evolución semanal." });
...
function historyBuckets(period:HistoryPeriod, today:string, entries:Entry[]) {
  ...
  return Array.from({length:count},(_,reverse) => {
    ...
    return {...bucket, macros: recorded ? metricKeys.reduce((result,key) => ({...result,[key]:total[key]/divisor}),{} as Macros) : null, total, recorded};
  });
}
function bodyWeeklyBuckets(entries: BodyComposition[]) {
  const currentWeek = nutritionWeek(localToday())[0];
  return Array.from({ length: 12 }, (_, index) => {
    const start = shiftDate(currentWeek, -(11-index)*7), end = shiftDate(start, 6);
    const measurement = entries.filter(entry => entry.recorded_at >= start && entry.recorded_at <= end).at(-1) ?? null;
    return { start, label:dateLabel(start,{day:"numeric",month:"short"}), measurement };
  });
}
...
export function NutritionView({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      db.from("body_composition_entries").select("id,recorded_at,weight,body_fat,muscle,fat_mass,lean_mass,body_water,bmi,basal_metabolic_rate").eq("user_id", userId).order("recorded_at"),
    ]).then(results => {
      if (!active) return;
  ...
  async function saveBodyComposition() {
    ...
    const calculatedLeanMass = bodyDraft.lean_mass ?? bodyDraft.weight * (1 - bodyDraft.body_fat / 100);
    const result = await db().from("body_composition_entries").insert({ ...bodyDraft, lean_mass: calculatedLeanMass, user_id: userId }).select().single();
  ...
  const detailChart = goals ? detailCardOrder.map(key => ({ key, label:detailMetrics[key], value:detailTotal[key], target:goals[key], direction:detailDirection[key] ?? "maximum", color:key, unit:detailMetricUnits[key], note:detailDirection[key] === "minimum" ? "mínimo diario" : "máximo diario" })) : [];
  ...
  const historyCalorieDelta = goals ? history.reduce((sum,item) => sum + (item.recorded ? item.total.calories - goals.calories * item.recorded : 0),0) : 0;
  ...
  const bodyValues = bodyWeeks.map(week => week.measurement ? (bodyMetric === "lean_mass" ? week.measurement.lean_mass ?? week.measurement.weight * (1-week.measurement.body_fat/100) : week.measurement[bodyMetric]) : null);
  const populatedBodyValues = bodyValues.filter((value): value is number => value !== null);
  const bodyMin = Math.min(...populatedBodyValues, 0), bodyMax = Math.max(...populatedBodyValues, 1), bodyRange = Math.max(bodyMax-bodyMin, bodyMetric === "body_fat" ? 1 : 2);
  const bodyPoints = bodyValues.map((value,index) => value === null ? null : `${index/(bodyWeeks.length-1)*100},${100-(value-(bodyMin-bodyRange*.12))/(bodyRange*1.24)*100}`).filter((value): value is string => value !== null).join(" ");
  ...
  return <section className="nutrition">
    ...
    {loading ? <p role="status">Cargando nutrición…</p> : <>
      ...
      <article className="panel nutrition-tips" aria-label="Consejos nutricionales del día"><div><p className="eyebrow">PÍLDORAS DEL DÍA</p><h2>Tu brújula nutricional</h2></div><div className="nutrition-tip-pills">{dailyTips.map(tip => <div className={tip.tone} key={tip.title}><b>{tip.title}</b><span>{tip.detail}</span><small>{tip.fact}</small></div>)}</div></article>
      ...
      <article className="panel nutrition-body"><div className="panel-head"><div><p className="eyebrow">COMPOSICIÓN CORPORAL</p><h2>Tu evolución semanal</h2></div><button disabled={busy} onClick={() => setBodyDraft(emptyBodyDraft())}>+ Añadir medición</button></div>{!bodyEntries.length ? <p>Añade una medición para generar objetivos diarios y ver tu evolución.</p> : <><div className="nutrition-body-summary">{(["weight","body_fat","muscle","lean_mass"] as BodyMetric[]).map(key => { const latest=bodyEntries.at(-1)!; const value=key === "lean_mass" ? (latest.lean_mass ?? latest.weight * (1-latest.body_fat/100)) : latest[key]; return <div key={key}><span>{bodyMetricMeta[key].label}</span><strong>{fmt(value)} {bodyMetricMeta[key].unit}</strong></div>; })}</div><div className="nutrition-history-tabs nutrition-body-tabs" role="tablist" aria-label="Métrica de composición corporal">{(Object.keys(bodyMetricMeta) as BodyMetric[]).map(key => <button key={key} role="tab" aria-selected={bodyMetric===key} className={bodyMetric===key ? `active ${key}` : ""} onClick={() => setBodyMetric(key)}>{bodyMetricMeta[key].label}</button>)}</div><div className={`nutrition-body-trend ${bodyMetric}`} aria-label={`Evolución semanal de ${bodyMetricMeta[bodyMetric].label}`}><div className="nutrition-body-scale"><span>{fmt(bodyMax)} {bodyMetricMeta[bodyMetric].unit}</span><span>{fmt(bodyMin)} {bodyMetricMeta[bodyMetric].unit}</span></div><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><line x1="0" y1="50" x2="100" y2="50" /><polyline points={bodyPoints} /></svg>{bodyWeeks.map((week,index) => <div key={week.start} className="nutrition-body-week" title={week.measurement ? `${week.label}: ${fmt(bodyValues[index] ?? 0)} ${bodyMetricMeta[bodyMetric].unit}` : `${week.label}: sin medición`}><i className={bodyValues[index] === null ? "empty" : ""} style={bodyValues[index] === null ? undefined : { top:`${100-(bodyValues[index]!-(bodyMin-bodyRange*.12))/(bodyRange*1.24)*100}%` }} /><small>{week.label}</small></div>)}</div><small>Se muestra la última medición de cada semana; las semanas sin medición no se estiman.</small></>}</article>
      ...
      {goals && historyRecorded.length > 0 && <article className={`panel nutrition-history-balance ${historyCalorieDelta <= 0 ? "deficit" : "surplus"}`}><p className="eyebrow">BALANCE DEL PERÍODO SELECCIONADO</p><h2>{historyCalorieDelta <= 0 ? "Déficit calórico" : "Exceso calórico"}</h2><strong>{historyCalorieDelta <= 0 ? "−" : "+"}{fmt(Math.abs(historyCalorieDelta))} <small>kcal</small></strong><p>{historyPeriod === "days" ? "Suma de los días registrados" : historyPeriod === "weeks" ? "Suma de las semanas mostradas" : "Suma de los meses mostrados"}. Solo cuenta períodos con comidas registradas.</p></article>}
    </>}
