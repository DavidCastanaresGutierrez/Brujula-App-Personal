...
export function NutritionView({ userId }: { userId: string }) {
  ...
  const editorKind = editor ? "meal" : favoriteEntry ? "favorite" : favoriteEditor ? "favorite-editor" : goalDraft ? "goals" : importOpen ? "import" : deleting ? "delete" : "";
  useEffect(() => {
  ...
  const detailChart = goals ? ([
    { key:"fiber", label:"Fibra", value:detailTotal.fiber, target:25, direction:"minimum", color:"fiber", note:"mínimo diario" },
    { key:"sugars", label:"Azúcares", value:detailTotal.sugars, target:goals.calories * .05 / 4, direction:"maximum", color:"sugars", note:"máx. de azúcares libres" },
    { key:"saturated_fat", label:"Grasas saturadas", value:detailTotal.saturated_fat, target:goals.calories * .1 / 9, direction:"maximum", color:"saturated", note:"máximo diario" },
    { key:"salt", label:"Sal", value:detailTotal.salt, target:5, direction:"maximum", color:"salt", note:"máximo diario" },
  ] as const) : [];
  ...
  function openMeal(meal: Meal = blankMeal(), id: string | null = null) { setFavoriteEntry(null); setFavoriteEditor(null); setGoalDraft(null); setImportOpen(false); setDeleting(null); setEditor({ name: meal.name, type: meal.type, quantity: meal.quantity, unit: meal.unit, calories: meal.calories, protein: meal.protein, carbs: meal.carbs, fat: meal.fat, fiber: meal.fiber, sugars: meal.sugars, saturated_fat: meal.saturated_fat, salt: meal.salt }); setEditingId(id); setError(""); }
