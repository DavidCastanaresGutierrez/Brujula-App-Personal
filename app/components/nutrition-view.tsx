...
export function NutritionView({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      db.from("frequent_meals").select("id,type,name,quantity,unit,calories,protein,carbs,fat,fiber,sugars,saturated_fat,salt").eq("user_id", userId).order("name"),
    ]).then(results => {
      if (!active) return;
  ...
  function openMeal(meal: Meal = blankMeal(), id: string | null = null) { setGoalDraft(null); setImportOpen(false); setDeleting(null); setEditor({ name: meal.name, type: meal.type, quantity: meal.quantity, unit: meal.unit, calories: meal.calories, protein: meal.protein, carbs: meal.carbs, fat: meal.fat, fiber: meal.fiber, sugars: meal.sugars, saturated_fat: meal.saturated_fat, salt: meal.salt }); setEditingId(id); setError(""); }
