...
export function NutritionView({ userId }: { userId: string }) {
  ...
  function openMeal(meal: Meal | FrequentMeal = blankMeal(), id: string | null = null) { const quantity = "quantity" in meal ? meal.quantity : 1; setGoalDraft(null); setImportOpen(false); setDeleting(null); setEditor({ name: meal.name, type: meal.type, quantity, unit: meal.unit, calories: meal.calories, protein: meal.protein, carbs: meal.carbs, fat: meal.fat, fiber: meal.fiber, sugars: meal.sugars, saturated_fat: meal.saturated_fat, salt: meal.salt }); setEditingId(id); setError(""); }
