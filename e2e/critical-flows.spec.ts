...
test("nutrición: objetivos, registro, edición, frecuentes e importación revisada", async ({ page }) => {
  ...
  await importForm.getByLabel("JSON nutricional").fill('{"schema_version":2}');
  await importForm.getByRole("button",{name:"Validar y previsualizar"}).click();
  ...
  await importForm.getByLabel("JSON nutricional").fill(json);
  await importForm.getByRole("button",{name:"Validar y previsualizar"}).click();
