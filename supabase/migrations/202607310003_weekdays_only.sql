...
begin
  ...
  insert into habits (user_id, id, category_id, kind, name, goal, color, position, archived, every_day, weekdays_only, celebrated_streak_30)
  select current_user_id, (item->>'id')::bigint, item->>'category', habit_kind, item->>'name',
         (item->>'goal')::integer, item->>'color', ordinality::integer - 1,
  ...
  from (
    ...
    from jsonb_array_elements(coalesce(payload->'daily', '[]'::jsonb)) with ordinality as daily_items(item, ordinality)
    union all
    select item, ordinality, 'weekly'::text as habit_kind
    from jsonb_array_elements(coalesce(payload->'weekly', '[]'::jsonb)) with ordinality as weekly_items(item, ordinality)
  ) all_habits;
